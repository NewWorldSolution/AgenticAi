...
client = OpenAI(
    ...
    api_key= "voc-218204610159874433027768e4f638a44c26.78307440"
)

def get_llm_response(system_prompt: str, user_prompt: str, model: str = "gpt-4o", temperature: float = 0.3):
    try:
...
def distillation_planner_agent(feedstock_analysis):
    ...
    """
    user_prompt = f"Based on this feedstock analysis, provide a distillation plan: {feedstock_analysis}. Example output: (Based on the analysis, potential yields are: Gasoline: 40%, Diesel: 30%, Kerosene: 20%, Other: 10%)"
...
if __name__ == "__main__":
    ...
    print(f"\n--- Market Data ---\n{market_data}\n")
    optimization_recommendation = production_optimizer_agent(distillation_plan, market_data)
