...
class PenguinAgent(ToolCallingAgent):
    def __init__(
        self, name: str, initial_food: int = 0, initial_has_tool: bool = False
    ) -> None:
        super().__init__(
            tools=[], model=model, name=name, description=f"A penguin named {name}"
        )
        self.name = name
    ...
    def take_action(self) -> Dict[str, Any]:
        ...
        print(
            f"Current State for {self.name}: Food: {self.food}, Has Toy: {self.has_toy}"
        )
        ...
        print(
            f"  {self.name} (Request): Type: {action_result.get('action_type')}, Details: '{action_result.get('details')}'"
        )
...
class ScientistAgent(ToolCallingAgent):
    ...
    def respond_to_action(
        ...
        print(
            f"Penguin State (before scientist action): Food: {penguin.food}, Has Toy: {penguin.has_toy}"
        )

        current_penguin_history = check_history(penguin.name)
        print(
            f"Penguin History (direct check for {penguin.name}): Recent Food Recorded: {current_penguin_history['recent_food']}, Has Been Given Tool (in history): {current_penguin_history['has_tool']}"
        )
        print(
        ...
        if record_distribution_tool_called_by_llm:
            ...
            if llm_intended_tool:
                if self.toy_available:
                    ...
                    print(
                        f"  INFO: Scientist LLM intended to give a toy to {penguin.name}, but no toy was available."
                    )
        ...
        print(
            f"Scientist Resources: Remaining Food Supply: {self.food_supply}, Toy Available: {self.toy_available}"
        )
        print(
            f"Penguin {penguin.name}: Food: {penguin.food}, Has Toy: {penguin.has_toy}"
        )
...
def run_simulation(num_rounds=3, num_penguins=2):
    ...
    for i in range(num_penguins):
        ...
        name_suffix = (
            str(i // len(penguin_names) + 1)
            if num_penguins > len(penguin_names) and i >= len(penguin_names)
            else ""
        )
        if num_penguins <= len(
            penguin_names
        ):  # if only 1 or 2 penguins, don't add suffix to first ones
            name_suffix = str(
                i + 1
            )  # Use 1-based indexing for names like miercoles1, putzie2
        else:  # For more penguins than names, use base + number
            name_suffix = (
                str(i // len(penguin_names) + 1)
                if i >= len(penguin_names)
                else str(i + 1)
            )
    ...
    print(
        f"Scientist Final Resources: Food: {scientist.food_supply}, Toy Available: {'🔨' if scientist.toy_available else 'No Toy'}"
    )
    ...
    for penguin in penguins:
        ...
        print(
            f"    Overall History (recorded by scientist): Recent Food: {history['recent_food']}, Had Tool Ever: {history['has_tool']}"
        )
