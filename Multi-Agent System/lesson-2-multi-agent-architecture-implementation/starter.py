...
def find_food(penguin_name: str, method: str) -> int:
    """
    Find food for the penguin using foraging or fishing methods.

    Args:
        penguin_name (str): The unique name or identifier of the penguin.
        method (str): The method to find food, either 'foraging' or 'fishing'.

    Returns:
        int: The amount of food found (0-7).
        foraging yields 0-3, fishing yields 2-7.
        otherwise 0.
    """
    method_1 = method.lower().strip()
    if method_1 == "foraging":
        return random.randint(0, 3)
    elif method_1 == "fishing":
...
class ScientistAgent(ToolCallingAgent):
    def __init__(
        self, initial_food_supply: int = 20, refresh_interval: int = 5
    ) -> None:
        super().__init__(
            tools=[
                check_history,
                record_distribution,
            ],  # Student might add check_history if they want scientist to use it
            model=model,
            name="scientist",
    ...
    def respond_to_action(
        self, penguin: "PenguinAgent", penguin_action_details: Dict[str, Any]
    ) -> None:
        ...
        if (
            self.turn_counter > 0
            and (self.turn_counter - 1) % self.refresh_interval == 0
            and self.turn_counter != 1
        ):  # Avoid refresh on very first turn unless interval is 1
        ...
        print(
            f"Penguin Action/Request: {penguin_action_details}"
        )  # Changed from penguin_action to penguin_action_details
        ...
        print(
            f"  - Has Been Given Tool (in history): {current_penguin_history['has_tool']}"
        )
        ...
        confirmation_msg_from_tool_execution = (
            "Tool 'record_distribution' output not found."
        )
                ...
                for tc in step.tool_calls:  # tc is a ToolCall object
                    if tc.name == "record_distribution":
                        ...
                        if (
                            ...
                            confirmation_msg_from_tool_execution = str(
                                step.observations
                            )
                        else:
        ...
        if record_distribution_tool_called_by_llm:
            ...
            print(
                f"  - Confirmation from tool execution (observation): {confirmation_msg_from_tool_execution}"
            )
        else:
            ...
            print(
                f"  No resources will be distributed by the scientist this turn due to this."
            )
        ...
        if (
            record_distribution_tool_called_by_llm
        ):  # Only proceed if LLM made a decision via the tool
            ...
            if food_to_attempt > 0:
                ...
                if actual_food_given < food_to_attempt:
                    print(
                        f"  INFO: Scientist intended {food_to_attempt} food, but only {actual_food_given} was available/given."
                    )
            ...
            if llm_intended_tool:  # If LLM decided to give a tool
                if self.tool_available:
                    ...
                    print(
                        f"  INFO: Scientist LLM intended to give a tool to {penguin.name}, but no tool was available."
                    )
...
class PenguinAgent(ToolCallingAgent):
    def __init__(
        self, name: str, initial_food: int = 0, initial_tool: bool = False
    ) -> None:  # Allow initial state
        # Student adds the find_food tool here as per the original starter
        super().__init__(
            tools=[find_food],
            model=model,
            name=name,
            description=f"A penguin named {name}",
        )
        self.name = name
    ...
    def take_action(self) -> Dict[str, Any]:
        ...
        print(
            f"Current State for {self.name}: Food: {self.food}, Has Tool: {self.has_tool}"
        )
        ...
        prompt = f"""
        You are a penguin named {self.name}.
        Your current state is:
        - Food: {self.food}
        - Has Tool: {self.has_tool}

        GOALS: survive by keeping your food supply healthy.

        You have exactly THREE way to act this turn:

        OPTION A) Find food yourself 
        - Use the tool 'find_food' exactly once if you choose this option.
        - Tool arguments MUST be:
        - penguin_name: "{self.name}"
        - method: "fishing" OR "foraging" 
        Decision guide:
        - If food is low (0-1): prefer method "fishing" (higher yield).
        - If food is moderate: prefer method "foraging".

        OPTION B) Request help from the scientist (text only)
        - If you want FOOD, write a short sentence that clearly contains the word "food".
        Example: "I need food."
        
        OPTION C) Request a tool from the scientist (text only)
        - If you want a TOOL, write a short sentence that clearly contains the word "tool".
        Example: "I would like a tool."

        IMPORTANT OUTPUT RULES:
        - If you use the 'find_food' tool, after the tool call your final text should be very short, e.g. "I tried fishing." or "I tried foraging."
        - If you do NOT use the tool, output ONLY the request sentence to the scientist (no extra explanation).

        Now decide the best action for this turn.
                """
        # --- END STUDENT TASK ---

        final_llm_text_response = self.run(
            prompt
        )  # LLM will populate self.memory.steps
            ...
            if hasattr(step, "tool_calls") and step.tool_calls:
                for tc in step.tool_calls:  # tc is a ToolCall object
                    if tc.name == "find_food":
                        find_food_method_argument = tc.arguments.get(
                            "method", "unknown"
                        )
                        ...
                        if (
                            ...
                            try:
                                food_value_from_tool_output = int(
                                    str(step.observations)
                                )  # Observations is the direct output
                                ...
                                print(
                                    f"  WARN [{self.name}]: 'find_food' output conversion error. Observation: '{step.observations}', Type: {type(step.observations)}, Error: {e}"
                                )
                        else:
                            ...
                            print(
                                f"  WARN [{self.name}]: 'find_food' tool called (ID {tc.id}), but 'observations' attribute missing or None on the ActionStep."
                            )
                        break
            if (
                find_food_tool_called_by_llm
            ):  # If we identified the call, no need to check further steps for this specific tool call
                break
        # ---- END: Robust parsing ----
        ...
        if find_food_tool_called_by_llm:
            ...
            if self.memory.steps:
                ...
                if (
                    hasattr(last_step, "tool_calls")
                    and last_step.tool_calls
                    and hasattr(last_step.tool_calls[0], "name")
                    and last_step.tool_calls[0].name == "final_answer"
                    and hasattr(last_step.tool_calls[0], "arguments")
                ):  # Check arguments exist
                    text_to_parse = str(
                        last_step.tool_calls[0].arguments.get(
                            "answer", final_llm_text_response
                        )
                    )

            response_lower = text_to_parse.lower()
            if "tool" in response_lower and (
                "request" in response_lower
                or "need" in response_lower
                or "want" in response_lower
                or "like" in response_lower
            ):
                action_result = {
                    "action_type": "request_tool",
                    "message": text_to_parse,
                }
            elif "food" in response_lower and (
                "request" in response_lower
                or "need" in response_lower
                or "want" in response_lower
                ...
                action_result = {
                    "action_type": "request_food",
                    "message": f"Ambiguous statement or no specific request: '{text_to_parse}'. Defaulting to request_food.",
                }
            print(
                f"  {self.name} (Text-Based Action Result): Type: {action_result.get('action_type')}, Msg: '{action_result.get('message')}'"
            )
...
def run_simulation(
    num_rounds=3, num_penguins=2
):  # Default to 2 penguins for simpler testing
    # Initialize Scientist
    scientist = ScientistAgent(
        initial_food_supply=20, refresh_interval=3
    )  # Smaller interval for testing refresh

    # Initialize Penguins
    penguins = [
        PenguinAgent(f"Pingu_{i+1}", initial_food=random.randint(0, 3))
    ...
    for round_num in range(num_rounds):
        ...
        current_penguin_actions: Dict[str, Dict[str, Any]] = (
            {}
    ...
    print(
        f"Scientist Final Resources: Food: {scientist.food_supply}, Tool Available: {scientist.tool_available}"
    )
    ...
    for penguin in penguins:
        ...
        if final_recorded_history:
            total_food_decided_for_penguin = sum(
                h["food"] for h in final_recorded_history
            )
            times_tool_decided_for_penguin = sum(
                1 for h in final_recorded_history if h["has_tool"]
            )
            print(
                f"    (Scientist's Records for {penguin.name}: {len(final_recorded_history)} decisions logged. "
                f"Total intended food: {total_food_decided_for_penguin}, "
                f"Times intended tool: {times_tool_decided_for_penguin})"
            )
        else:
            print(
                f"    (Scientist's Records for {penguin.name}: No distribution decisions logged by scientist)"
            )
