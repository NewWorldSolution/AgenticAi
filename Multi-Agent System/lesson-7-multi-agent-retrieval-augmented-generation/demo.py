...
class AccessControl:
    ...
    def can_access(requester_level: str, data_level: str) -> bool:
        ...
        if (
            requester_level == PrivacyLevel.FINANCIAL
            and data_level == PrivacyLevel.AGENT
        ):
            return True

        # Agent can access customer data
        if (
            requester_level == PrivacyLevel.AGENT
            and data_level == PrivacyLevel.CUSTOMER
        ):
            return True
...
class Claim:
    ...
    def add_complaint(self, complaint_text: str):
        ...
        self.complaint_history.append(
            {"timestamp": datetime.now().isoformat(), "text": complaint_text}
        )
...
class Database:
    ...
    def search_similar_claims(
        self, claim_dict: Dict, requester_level: str, threshold: float = 0.7
    ) -> List[Dict]:
        ...
        for claim in self.claims.values():
            ...
            if score >= threshold:
                results.append(
                    {"claim": claim.to_dict(requester_level), "similarity_score": score}
                )
...
class VectorKnowledgeBase:
    ...
    def search(self, query, access_level, threshold=0.15):
        ...
        for i, score in enumerate(similarities):
            ...
            if score >= threshold and AccessControl.can_access(
                access_level, entry["privacy_level"]
            ):
...
class VectorClaimSearch:
    ...
    def search(self, query_claim, requester_level, threshold=0.2):
        ...
        # Prepare query text
        procedure_desc = database.procedure_codes.get(
            query_claim.get("procedure_code", ""), ""
        )
...
class DataGenerator:
    ...
    def populate_database(
        num_patients: int = 50, num_claims: int = 200, num_complaints: int = 40
    ):
        ...
        for _ in range(min(num_complaints, len(denied_claims))):
            ...
            if random.random() < 0.7:
                ...
                for i in range(num_responses):
                    complaint.add_response(
                        random.choice(responses), "Customer Service Rep"
                    )
...
def retrieve_claim_history(
    patient_id: int, access_level: str = PrivacyLevel.AGENT
) -> Dict:
    """
    ...
    return {"patient_id": patient_id, "claims_count": len(claims), "claims": claims}
...
def respond_to_complaint(
    complaint_id: str, response: str, responder: str, resolve: bool = False
) -> Dict:
    """
...
def get_complaint_history(
    complaint_id: str, access_level: str = PrivacyLevel.AGENT
) -> Dict:
    """
...
class ComplaintResolutionOrchestrator(ToolCallingAgent):
    ...
    def __init__(self, model: OpenAIServerModel):
        ...
        def handle_customer_complaint(
            patient_id: int, complaint_text: str, claim_id: str = None
        ) -> Dict:
            """
            ...
            if not claim_id:
                ...
                if "CLM-" in text_content:
                    ...
                    if (
                        potential_claim_id.startswith("CLM-")
                        and len(potential_claim_id) >= 9
                    ):
            ...
            if hasattr(complaint_result, "tool_calls") and complaint_result.tool_calls:
                for call in complaint_result.tool_calls:
                    if (
                        call.name == "submit_complaint"
                        and "complaint_id" in call.arguments
                    ):
            ...
            if hasattr(medical_review, "tool_calls") and medical_review.tool_calls:
                for call in medical_review.tool_calls:
                    if (
                        call.name == "respond_to_complaint"
                        and "response" in call.arguments
                    ):
            ...
            if not resolution:
                complaint_info = database.get_complaint(
                    complaint_id, PrivacyLevel.ADMIN
                )
            ...
            if not resolution:
                ...
                if (
                    "apologize" in text_content.lower()
        ...
        def generate_random_complaint() -> Dict:
            ...
            denied_claims = [
                c for c in database.claims.values() if c.status == "denied"
            ]
            if not denied_claims:
                return {"success": False, "error": "No denied claims in database"}
...
def run_demo():
    ...
    if resolution_info and resolution_info.get("success", False):
        ...
        resolution_text = resolution_info.get(
...
if __name__ == "__main__":
    ...
    if (
        demo_result is not False
    ):  # Only show this message if the demo didn't explicitly fail
        print(
